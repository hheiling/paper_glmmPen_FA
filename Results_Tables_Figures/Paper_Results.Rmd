---
title: "Paper Results"
author: "Hillary Heiling"
date: "10/13/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
```


# Random Effects Covariance Matrices used in Simulations

$r = 3$ simulations 'Large' $B$ matrix:

```{r, echo=TRUE}
q = 11
B0 = cbind(rep(1,q),
          c(0, rep(c(-1, 1), each = 5)),
          c(-2, 2, rep(c(-1, 0, 1), times = 3)))
```

Moderate $B$ matrix: covariance and eigenvalues

```{r, echo=TRUE}
(B = B0 * 0.75)
# Covariance matrix
cov_mat = B %*% t(B)
print(round(diag(cov_mat), 2))
# Calculate eigenvalues
SVD = svd(cov_mat)
round(SVD$d,2)
```

Large $B$ matrix: covariance and eigenvalues

```{r, echo=TRUE}
(B = B0)
# Covariance matrix
cov_mat = B %*% t(B)
print(round(diag(cov_mat), 2))
# Calculate eigenvalues
SVD = svd(cov_mat)
round(SVD$d,2)
```


$r=5$ simulations 'Large' $B$ matrix:

```{r, echo=TRUE}
q = 11
B0 = cbind(rep(1,q),
          c(0, rep(c(-1, 1), each = 5)),
          c(-1, 1, rep(c(-1, 0, 1), times = 3)),
          c(rep(c(-1, 1, 1, -1), times = 2), -1, 1, -1),
          c(rep(c(-1, -1, 0, 1, 1), times = 2), -2))

```

Moderate $B$ matrix: covariance and eigenvalues

```{r, echo=TRUE}
(B = B0 * 0.8)
# Covariance matrix
cov_mat = B %*% t(B)
print(round(diag(cov_mat), 2))
# Calculate eigenvalues
SVD = svd(cov_mat)
round(SVD$d,2)
```

Large $B$ matrix: covariance and eigenvalues

```{r, echo=TRUE}
(B = B0)
# Covariance matrix
cov_mat = B %*% t(B)
print(round(diag(cov_mat), 2))
# Calculate eigenvalues
SVD = svd(cov_mat)
round(SVD$d,2)

```


```{r, message=FALSE, warning=FALSE}
# Load relevant libraries
library(stringr)
library(xtable)
```


```{r}
# Extract relevant information from simulation results
extract = function(path, sims, sim_type,
                   q = 101, p_true = 11, r_true_vec = 3, trace = 0,
                   beta_true = rep(1,length(sims))){
  
  # Results of r estimation procedure
  ## Percent of times too small, too large, or correct
  rest_pseudo = matrix(0, nrow = length(sims), ncol = 3)
  rownames(rest_pseudo) = sim_type
  colnames(rest_pseudo) = c("Too_small_Pct","Correct_Pct","Too_Large_Pct")
  ## Average estimate of r
  rest_avg_pseudo = numeric(length(sims))
  names(rest_avg_pseudo) = sim_type
  
  rest_lst = list()
  
  # Save summary output in out_mat
  ## labs = labels of columns of out_mat
  labs = c(str_c("Beta",1:(p_true-1)),"TP % Fixef","FP % Fixef",
             "TP % Ranef","FP % Ranef",
             "Median Time (hrs)","Abs. Dev. (Mean)")
  out_mat = matrix(0, nrow = length(sims), ncol = length(labs))
  colnames(out_mat) = labs
  rownames(out_mat) = sim_type
  
  # Save additional variance and fixed effect information
  vars_lst = list()
  beta_lst = list()
  
  r_input_lst = list()
  y_lst = list()
  
  # Save eigenvalues and true variances of random effect covariance matrix used in simulations
  eigen_values = matrix(0, nrow = length(sims), ncol = max(r_true_vec))
  rownames(eigen_values) = sim_type
  vars_true = matrix(0, nrow = length(sims), ncol = p_true)
  rownames(vars_true) = sim_type
  
  # Average values for all fixed effects selected in best model as well as
  ## average variances selected in best models
  beta_all_avg = matrix(0, nrow = length(sims), ncol = q)
  rownames(beta_all_avg) = sim_type
  colnames(beta_all_avg) = str_c("Beta",0:(q-1))
  
  vars_all_avg = beta_all_avg
  colnames(vars_all_avg) = str_c("Var",0:(q-1))
  
  # Save bias information
  abs_dev_lst = list()
  # Save time information
  time_lst = list()
  
  for(m in 1:length(sims)){
    
    files = list.files(path = str_c(path, sims[m]), full.names = T)
    if(trace == 1){
      cat("completed replicates: ", length(files), "\n")
    }
    
    if(length(r_true_vec) == 1){
      r_true = r_true_vec
    }else if(length(r_true_vec) > 1){
      r_true = r_true_vec[m]
    }
    
    time_mat = numeric(length(files))
    EM_iter_lst = list()
    r_est = numeric(length(files))
    
    beta_mat = matrix(0, nrow = length(files), ncol = q)
    abs_dev = numeric(length(files)) # Mean absolute deviation
    vars_mat = matrix(0, nrow = length(files), ncol = q)
    presc_mat = matrix(0, nrow = length(files), ncol = q)
    EM_iter = NULL
    opt_res = NULL
    select_res = list()
    r_input = numeric(length(files))
    y_summary = matrix(NA, nrow = length(files), ncol = 6)
    
    for(f in 1:length(files)){
      # load output list object
      load(files[f])
      beta_mat[f,] = output$coef_mat
      beta_vec = output$coef_mat[,c(2:p_true)] # Ignore intercept in absolute deviation calculation
      beta_non0 = beta_vec[which(beta_vec != 0)]
      if(length(beta_non0) >= 1){
        abs_dev[f] = mean(abs(beta_non0 - rep(beta_true[m],times=length(beta_non0))))
      }else{
        abs_dev[f] = NA
      }
      vars_mat[f,] = output$vars_mat
      presc_mat[f,] = output$presc_mat
      opt_res = rbind(opt_res, output$results_optim[[1]])
      EM_iter = rbind(EM_iter,output$selection_results[,"EM_iter"])
      time_mat[f] = output$time_mat[1,3]
      select_res[[f]] = output$selection_results
      if(!is.null(output$r_est)){
        r_est[f] = output$r_est
      }
      y_tmp = output$y
      if(!is.null(y_tmp)){
        y_summary[f,] = summary(y_tmp)[1:6]
        if(f == 1){
          colnames(y_summary) = names(summary(y_tmp))[1:6]
        }
      }
      
      if(!is.null(output$r_input)){
        r_input[f] = output$r_input
      }else{
        r_input[f] = NA
      }
      
    } # End f for loop
    
    abs_dev_lst[[m]] = abs_dev
    
    EM_iter_lst[[m]] = EM_iter
    
    time_lst[[m]] = time_mat
    
    # True and false positives - fixed effects, random effects,
    # "TP PreSc","FP Presc","Med Presc"
    out = numeric(length(labs))
    names(out) = labs
    
    
    for(i in 2:p_true){
      out[i-1] = mean(beta_mat[which(beta_mat[,i] != 0),i])
    }
    for(i in 1:q){
      # Note: will be NA if never != 0
      beta_all_avg[m,i] = mean(beta_mat[which(beta_mat[,i] != 0),i])
      vars_all_avg[m,i] = mean(vars_mat[which(vars_mat[,i] != 0),i])
    }
    out[p_true] = sum(beta_mat[,c(2:p_true)] != 0) / length(files) * (1 / (p_true-1) * 100)
    out[p_true+1] = sum(beta_mat[,-c(1:p_true)] != 0) / length(files) * (1 / (q - p_true) * 100)
    out[p_true+2] = sum(vars_mat[,c(2:p_true)] != 0) / length(files) * (1 / (p_true-1) * 100)
    out[p_true+3] = sum(vars_mat[,-c(1:p_true)] != 0) / length(files)  * (1 / (q - p_true) * 100)
    out[p_true+4] = median(time_mat / 3600)
    out[p_true+5] = mean(abs_dev, na.rm = TRUE)
    
    out_mat[m,] = out
    
    # summary of r estimation using pseudo random effects estimates
    rest_pseudo[m,1] = sum(r_est < r_true) / length(files) * 100
    rest_pseudo[m,2] = sum(r_est == r_true) / length(files) * 100
    rest_pseudo[m,3] = sum(r_est > r_true) / length(files) * 100
    
    rest_avg_pseudo[m] = mean(r_est)
    
    rest_lst[[m]] = r_est
    vars_lst[[m]] = vars_mat
    beta_lst[[m]] = beta_mat
    r_input_lst[[m]] = r_input

    y_lst[[m]] = y_summary
    
    eigen_values[m,] = output$eigen_vals[1:max(r_true_vec)]
    B_true = output$B_true
    vars_true[m,] = diag(B_true %*% t(B_true))
  }
  
  
  
  out_mat = round(out_mat, digits = 2)
  rest_pseudo = round(rest_pseudo, digits = 2)
  rest_avg_pseudo = round(rest_avg_pseudo, 2)
  
  vars_val = round(diag(output$B_true %*% t(output$B_true)), digits = 2)
  
  return(list(out_mat = out_mat, 
              EM_iter = EM_iter_lst, 
              rest_pseudo = rest_pseudo,
              rest_avg_pseudo = rest_avg_pseudo,
              vars_val = vars_val, rest_lst = rest_lst,
              vars_lst = vars_lst, beta_lst = beta_lst,
              beta_all_avg = beta_all_avg, vars_all_avg = vars_all_avg,
              r_input = r_input_lst,
              y_lst = y_lst,
              eigen_values = eigen_values,
              vars_true = vars_true,
              time_lst = time_lst,
              abs_dev_lst = abs_dev_lst))
  
}
```

# Variable selection for Binomial data, p=100 - Version 3

```{r}
# True number of predictors with non-zero fixed and random effects in these simulations
p_true = 11
```

```{r}
# Note: For others interested in re-creating the output here, replace the code below with the line:
# load("Paper_Results/select100_glmmPen_FA.RData")
# This will load the 'res' list object with the relevant output

# Path to full simulation results
path = "~/Longleaf/Project2/Simulation_Results/select100_glmmPen_FA/"
# Sub-folders with individual simulation results
sims = str_c("sim",1:16)
# Description of individual simulation results
sim_type = str_c(rep(c("r3_","r5_"),each=8),
                 "Beta_",rep(rep(c(1,2),each=4),times=2),
                 rep(rep(c("_Moderate_","_Large_"),each=2),times=4),
                 rep(c("r_GR","r_True"),times=8))
res = extract(path = path, sims = sims, sim_type = sim_type,
              q = 101, p_true = 11, r_true_vec =c(rep(3,8),rep(5,8)), 
              trace = 0, beta_true = rep(rep(c(1,2),each=4),times=2))
save(res, file = "Paper_Results/select100_glmmPen_FA.RData")
```


### Comparison of r estimation on psuedo random effects

Average $r$ and percent of times r underestimated, overestimated, and estimated correctly

```{r}
# Only examine results for simulations that used the Growth Ratio estimation procedure
# to calculate r
idx = seq(from = 1, to = 16, by = 2)
data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)[idx,]
```

### Algorithm Performance

True and false positive rates:

```{r}
res$out_mat[,-c(1:(p_true-1))]
```

Tables used in paper:

```{r}
# latex tables

# True/False Positives and Timing
print(xtable(res$out_mat[,-c(1:(p_true-1))]))

# r estimate results
r_out = data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)
print(xtable(r_out[idx,], digits = c(0,2,0,0,0)))
```

Figures: Bar plot for TP and FP

```{r}
library(ggplot2)
library(reshape2)
df0 = as.data.frame(res$out_mat[seq(from = 1, to = 16, by = 2),c(11:14)])
# sim_type0 = rep(str_c("B=", rep(c(1,2), each = 2),", ",
#                       rep(c("Large","Moderate"), times = 2)), 
#                 times = 2)
sim_type0 = rep(str_c(expression(beta),"=", rep(c(1,2), each = 2),", ",
                      rep(c("Moderate","Large"), times = 2)), 
                times = 2)

df0$sim_type = factor(sim_type0, levels = c("beta=1, Moderate","beta=1, Large",
                                            "beta=2, Moderate","beta=2, Large"))
df0$r = rep(c("r = 3","r = 5"), each = 4)

df = melt(df0, id.vars = c("sim_type","r"))
# df$sim_type = factor(df$sim_type, levels = c("B=2, Large","B=2, Moderate",
#                                              "B=1, Large","B=1, Moderate"))
# df$Positive = str_sub(df$variable, start = 1, end = 2)
df$Positive = ifelse(str_detect(df$variable,"TP"),"True Positives %","False Positives %")
df$Positive <- factor(df$Positive, levels = c("True Positives %","False Positives %"))
# df$Type = str_sub(df$variable, start = 4, end = 9)
df$Type = ifelse(str_detect(df$variable,"Fixef"),"Fixed Effects","Random Effects")
# df
# df$Positive

ggplot(data = df, mapping = aes(x = sim_type, y = value, fill = Positive)) + 
  geom_col(position = position_dodge2()) + # geom_col(position = position_dodge2(reverse = TRUE))
  facet_grid(r ~ Type) +
  geom_text(aes(label = value), 
            position = position_dodge(width = 1),
            vjust = -0.5) +
  xlab("Simulation Set-up") +
  ylab("Average Selected Predictors") + 
  scale_fill_manual(values = c("deepskyblue","salmon")) +
  scale_y_continuous(breaks = seq(0, 100, 20), limits = c(0,120)) + 
  scale_x_discrete(labels = c(expression(paste(beta," = 1, ",Sigma, " = Moderate")),
                              expression(paste(beta," = 1, ",Sigma, " = Large")),
                              expression(paste(beta," = 2, ",Sigma, " = Moderate")),
                              expression(paste(beta," = 2, ",Sigma, " = Large")))) +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 330),
        legend.position = "top",
        legend.title = element_blank())
  
ggsave(filename = "Figures/Positives Figure Paper Percents.jpeg",
       height = 5, width = 7, units = "in")

# geom_hline(yintercept = 10, color = "red", linetype = "dashed") +
# scale_y_continuous(breaks = seq(0, 12, 2), limits = c(0,11)) +

```



# glmmPen vs glmmPen\_FA: Variable selection for Binomial data, $r=3$, $p=25$ 

All glmmPen\_FA simulations used Growth Ratio estimate of $r$

```{r}
# True number of predictors with non-zero fixed and random effects in these simulations
p_true = 11
```


```{r}
# Note: For others interested in re-creating the output here, replace the code below with the line:
# load("Paper_Results/select025_comparison.RData")
# This will load the 'res' list object with the relevant output

# Path to full simulation results
path = "~/Longleaf/Project2/Simulation_Results/select025_comparison/"
# Sub-folders with individual simulation results
sims = str_c("sim",1:8) 
# Description of individual simulation results
sim_type = str_c("Beta_",rep(c(1,2),each=4),
                 rep(rep(c("_Moderate_","_Large_"),each=2),times=2),
                 rep(c("glmmPen_FA_","glmmPen_"),times=4))
res = extract(path = path, sims = sims, sim_type = sim_type,
              q = 26, p_true = 11, r_true_vec =3, trace = 0,
              beta_true = rep(rep(c(1,2),each=4),times=2))
save(res, file = "Paper_Results/select025_comparison.RData")
```


### Comparison of r estimation on psuedo random effects: True r = 3

Average $r$ and percent of times r underestimated, overestimated, and estimated correctly

```{r}
# Only examine results for simulations that used the Growth Ratio estimation procedure
# to calculate r
idx = seq(from = 1, to = 8, by = 2)
data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)[idx,]
```


### Algorithm Performance

True and false positive rates:

```{r}
res$out_mat[,-c(1:(p_true-1))]
```

Tables used in paper:

```{r}
# latex tables

# True and false positives, timing, and average of the mean abs. deviation
print(xtable(res$out_mat[,-c(1:(p_true-1))]))

# r estimate results
r_out = data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)
print(xtable(r_out[idx,], digits = c(0,2,0,0,0)))
```


# glmmPen $p=100$: Variable selection for Binomial data, $r=3$

```{r}
# True number of predictors with non-zero fixed and random effects in these simulations
p_true = 11
```

```{r}
# Note: For others interested in re-creating the output here, replace the code below with the line:
# load("Paper_Results/select100_glmmPen.RData")
# This will load the 'res' list object with the relevant output

# Path to full simulation results
path = "~/Longleaf/Project2/Simulation_Results/select100_glmmPen/"
# Sub-folders with individual simulation results
sims = str_c("sim",1:4)
# Description of individual simulation results
sim_type = str_c("glmmPen",
                 "_Beta_",rep(c(1,2),each=2),
                 rep(c("Moderate","Large"),times=2))
res = extract(path = path, sims = sims, sim_type = sim_type,
              q = 101, p_true = 11, r_true_vec =c(rep(3,8),rep(5,8)), 
              trace = 0, beta_true = rep(rep(c(1,2),each=4),times=2))
save(res, file = "Paper_Results/select100_glmmPen.RData")
```


### Number completed

```{r}
for(i in 1:length(sim_type)){
  print(sprintf("%s completed in 100 hrs: %i", sim_type[i], nrow(res$beta_lst[[i]])))
}
```


### Minimum times for those completed

```{r}
for(i in 1:length(sim_type)){
  print(sprintf("%s min time: %.2f", sim_type[i], round(min(res$time_lst[[i]] / 3600),2)))
}
```

# Variable selection for Poisson data, $p=100$, $r=3$

All simulations used Growth Ratio estimate of $r$

```{r}
# True number of predictors with non-zero fixed and random effects in these simulations
p_true = 6
```

```{r}
# Note: For others interested in re-creating the output here, replace the code below with the line:
# load("Paper_Results/select100_Poisson.RData")
# This will load the 'res' list object with the relevant output

# Path to full simulation results
path = "~/Longleaf/Project2/Simulation_Results/Poisson_glmmPen_FA_v2/"
# Sub-folders with individual simulation results
sims = str_c("sim",1)
# Description of individual simulation results
sim_type = "Poisson_r3_Moderate"
res = extract(path = path, sims = sims, sim_type = sim_type,
              q = 101, p_true = 6, r_true_vec = 3,
              trace = 0, beta_true = 1)
save(res, file = "Paper_Results/select100_Poisson.RData")
```


### Comparison of r estimation on psuedo random effects

Average $r$ and Percent of times r underestimated, overestimated, and estimated correctly

```{r}
# Examine results for simulations that used the Growth Ratio estimation procedure
# to calculate r
data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)
```

### Algorithm Performance using r estimated from psuedo estimates

True and false positive rates:

```{r}
res$out_mat[,-c(1:(p_true-1))]
```

# Variable selection for Binomial data, $p=500$, $r=3$

All simulations used Growth Ratio estimate of $r$

```{r}
# True number of predictors with non-zero fixed and random effects in these simulations
p_true = 11
```

```{r}
# Note: For others interested in re-creating the output here, replace the code below with the line:
# load("Paper_Results/select500_glmmPen_FA.RData")
# This will load the 'res' list object with the relevant output

# Path to full simulation results
path = "~/Longleaf/Project2/Simulation_Results/select500_glmmPen_FA_v3/"
# Sub-folders with individual simulation results
sims = str_c("sim",1:4)
# Description of individual simulation results
# r3_Beta_1_Moderate
sim_type = str_c("r3_Beta_",
                 rep(c(1,2),each=2),
                 "_",rep(c("Moderate","Large"), times=2))
res = extract(path = path, sims = sims, sim_type = sim_type,
              q = 501, p_true = 11, r_true_vec = 3,
              trace = 0, beta_true = rep(c(1,2),each=2))
save(res, file = "Paper_Results/select500_glmmPen_FA.RData")
```


### Comparison of r estimation on psuedo random effects: True r = 3

Average $r$ and Percent of times r underestimated, overestimated, and estimated correctly

```{r}
idx = 1:4
data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)
```

### Algorithm Performance using r estimated from psuedo estimates

True and false positive rates:

```{r}
res$out_mat[,-c(1:(p_true-1))]
```
<!-- Version 2: -->
<!-- r3_Beta_1_Moderate      97.40       1.26       92.6       0.08             16.90             0.28 -->
<!-- r3_Beta_1_Large         92.58       1.69       93.3       0.38             28.78             0.41 -->
<!-- r3_Beta_2_Moderate     100.00       1.02       76.3       0.05             10.54             0.44 -->
<!-- r3_Beta_2_Large         99.90       1.25       86.0       0.12             16.17             0.53 -->

<!-- Version 3: -->
<!-- r3_Beta_1_Moderate       97.4       0.97       93.9       0.05             10.37             0.27 -->
<!-- r3_Beta_1_Large          88.5       1.73       94.3       0.33             17.57             0.37 -->
<!-- r3_Beta_2_Moderate      100.0       0.06       77.4       0.00             11.44             0.48 -->
<!-- r3_Beta_2_Large          99.6       0.19       88.1       0.03             13.22             0.55 -->

Tables used in paper:

```{r}
# latex tables

# True/False Positives and Timing
print(xtable(res$out_mat[,-c(1:(p_true-1))]))

# r estimate results
r_out = data.frame(Avg_r = res$rest_avg_pseudo, res$rest_pseudo)
print(xtable(r_out[idx,], digits = c(0,2,0,0,0)))
```

Figures: Bar plot for TP and FP

```{r}
library(ggplot2)
library(reshape2)
df0 = as.data.frame(res$out_mat[seq(from = 1, to = 4, by = 1),c(11:14)])
# sim_type0 = rep(str_c("B=", rep(c(1,2), each = 2),", ",
#                       rep(c("Large","Moderate"), times = 2)), 
#                 times = 2)
sim_type0 = str_c(expression(beta),"=", rep(c(1,2), each = 2),", ",
                      rep(c("Moderate","Large"), times = 2))

df0$sim_type = factor(sim_type0, levels = c("beta=1, Moderate","beta=1, Large",
                                            "beta=2, Moderate","beta=2, Large"))
df0$r = rep(c("r = 3"), times = 4)

df = melt(df0, id.vars = c("sim_type","r"))
df$Positive = ifelse(str_detect(df$variable,"TP"),"True Positives %","False Positives %")
df$Positive <- factor(df$Positive, levels = c("True Positives %","False Positives %"))
df$Type = ifelse(str_detect(df$variable,"Fixef"),"Fixed Effects","Random Effects")

ggplot(data = df, mapping = aes(x = sim_type, y = value, fill = Positive)) + 
  geom_col(position = position_dodge2()) + # geom_col(position = position_dodge2(reverse = TRUE))
  facet_grid(r ~ Type) +
  geom_text(aes(label = value), 
            position = position_dodge(width = 1),
            vjust = -0.5) +
  xlab("Simulation Set-up") +
  ylab("Average Selected Predictors") + 
  scale_fill_manual(values = c("deepskyblue","salmon")) +
  scale_y_continuous(breaks = seq(0, 100, 20), limits = c(0,120)) +
  scale_x_discrete(labels = c(expression(paste(beta," = 1, ",Sigma, " = Moderate")),
                              expression(paste(beta," = 1, ",Sigma, " = Large")),
                              expression(paste(beta," = 2, ",Sigma, " = Moderate")),
                              expression(paste(beta," = 2, ",Sigma, " = Large")))) +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 330),
        legend.position = "top",
        legend.title = element_blank())
  
ggsave(filename = "Figures/Positives Figure p 500 Percent.jpeg",
       height = 4, width = 7, units = "in")

```

The End
