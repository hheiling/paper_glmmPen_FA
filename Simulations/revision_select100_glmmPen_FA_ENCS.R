...
## Elastic net, where predictor correlation is based on case study correlation
...
# Arrays 1-500 - 5 simulation types, 100 replicates per simulation type
...
sub_folder = "select100_glmmPen_FA_ENCS/" # EN = elastic net
...
prefix0_post = str_c(header_work, "Project2_Revision/",sub_folder)
...
# 50 simulation replicates (to start, increase to 100 later)
sim_total = 100
...
B_size = c("moderate") # "moderate","large"
# Value of non-zero slopes
beta_val = c(1) # 1,2
# number of latent common factors
r_val = c(3) 
# type of correlation - only consider correlation based on case study data
cor_type = c("CS")
# value of alpha to consider
alpha_val = c(0.1, 0.3, 0.5, 0.8, 1.0)
# All simulation combinations
combos = expand.grid(est_type, B_size, beta_val, r_val, cor_type, alpha_val)
colnames(combos) = c("type", "B_size", "beta_val", "r_val", "cor_type", "alpha_val")
combos
...
cor_type = combos[sim_num,"cor_type"]
alpha = combos[sim_num,"alpha_val"]

# Correlation details
if(cor_type == "CS"){
  # Base correlations off of existing dataset
  ## load PDAC_basal dataset
  load(sprintf("%s/PDAC_basal.RData",path_data)) # load cluster covariates
  ## extract covariates, calculate spearman correlation
  X_pdac = PDAC_basal[,-c(1:3)]
  cor_mat = cor(X_pdac, method = "spearman")
  ## Of the 110 cluster covariates, select correlations from 100 of them
  ## Note: The first 10 values in the sequence indicate the 10 predictors that will be
  ## given non-zero fixed and random effects
  set.seed(2023)
  clust_num = sample(1:ncol(X_pdac), size = 100, replace = FALSE) 
  cor_use = cor_mat[clust_num, clust_num]
}
