...
# Arrays 1-100 - 1 simulation types, 100 replicates per simulation type
...
sub_folder = "revision_alt_num_ranef_glmmPen_FA_02/"
...
source(str_c(header_nas, "Project2_Revision/code/sim_generation_FA_alt.R"))
...
p = 101
# q_f = number of true predictors with non-zero fixed effects, plus intercept
q_f = 11
# q_r = number of true predictors with non-zero random effects, plus intercept
## Subset of fixed effects
q_r = 6
...
combos = expand.grid(est_type, B_size, beta_val, r_val)
colnames(combos) = c("type", "B_size", "beta_val", "r_val")
combos
...
# Determine B matrix to use for simulation. Dimension: q_r rows, r columns
...
dat = sim.data.FA.alt(n = N, ptot = p-1, pnonzero = q_f-1, nstudies = K, sd_raneff = 0,
                  family = "binomial", B = B, r=r,
                  seed=seeds[batch], imbalance = 0, 
                  beta = c(intercept,rep(beta,times=q_f-1)),
                  pnonzerovar = 0, pzerovar = q_r-1) 
