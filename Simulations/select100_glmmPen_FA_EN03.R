...
library(glmmPen) # Version 1.5.3.0
library(stringr)

# Arrays 1-2000 - 20 simulation types, 100 replicates per simulation type
...
sub_folder = "select100_glmmPen_FA_EN03/" # EN = elastic net
...
prefix0_post = str_c(header_work, "Project2/",sub_folder)
...
cor_val = combos[sim_num,"cor_type"]
...
if(!is.na(cor_val)){
  ...
  # For cor_type NA case, base correlations off of existing dataset
  ## load PDAC_basal dataset
  load("PDAC_basal.RData") # load cluster covariates
  ...
  set.seed(2023)
  clust_num = sample(1:110, size = 100, replace = FALSE) # clust_num = clust_num[order(clust_num)]
