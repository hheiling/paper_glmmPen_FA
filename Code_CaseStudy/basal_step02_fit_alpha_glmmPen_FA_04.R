...
sub_folder = "PDAC_basal_alpha_glmmPen_FA_04/"
# prefix: folder to save results
## prefix: location to save main output files
prefix = str_c(header_nas, "Project2/",sub_folder)
...
prefix_BICq = str_c(header_work, "Project2/",sub_folder)
...
load(file = "PDAC_basal.RData") # Location: home directory
...
if(p == 50){
  set.seed(2023)
  clust_samp = sample(1:110, size = p, replace = FALSE)
  X = as.matrix(PDAC_basal[,c(clust_samp + 3)])
}else{ # All available covariates
  X = as.matrix(PDAC_basal[,-c(1:3)])
}

# Check dimension of X: Should have 360 rows, p columns
dim(X)
